  ...
  <div class="card">
    ...
    <div class="card-datatable table-responsive pt-0">
      <table class="table text-center" id="user-list-table">
        ...
        <tr>
        <td>{{$i+1}}</td>
        <td>{{$start_date[$i]}} - {{$end_date[$i]}}</td>
        <td><a class='btn btn-success' href="trackerInfoByDate?s_d={{base64_encode($start_date[$i])}}&e_d={{base64_encode($end_date[$i])}}&p_={{base64_encode($projectname['id'])}}&u_={{base64_encode($username['id'])}}">Show Details</a></td>
        </tr>
